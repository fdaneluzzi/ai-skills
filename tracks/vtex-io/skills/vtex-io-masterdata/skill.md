...
description: >
  Apply when working with MasterData v2 entities, schemas, or MasterDataClient in VTEX IO apps. Covers
  data entities, JSON Schema definitions, CRUD operations, the masterdata builder, triggers, search and
  scroll operations, and schema lifecycle management. Use for storing, querying, and managing custom data
  in VTEX IO apps while avoiding the 60-schema limit through proper schema versioning.
metadata:
  track: vtex-io
  tags:
    - vtex-io
  ...
  globs:
    - "**/masterdata/**/*.ts"
    - "masterdata/**/*.json"
    - "node/clients/**/*MasterData*.ts"
  version: "1.0"
  purpose: Integrate Master Data v2 with VTEX IO apps for custom data storage and retrieval
  applies_to:
    - storing custom data (reviews, wishlists, form submissions)
    - querying and filtering Master Data documents
    - defining data entity schemas with JSON Schema
    - configuring Master Data triggers
  excludes:
    - general backend service patterns (see vtex-io-service-apps)
    - GraphQL schema definitions (see vtex-io-graphql-api)
  decision_scope:
    - masterdata builder vs API-created schemas
    - searchDocuments vs scrollDocuments
    - schema lifecycle and the 60-schema limit
