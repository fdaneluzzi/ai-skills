...
metadata:
  ...
  globs:
    ...
    - "node/resolvers/**/*.ts"
  version: "1.0"
  purpose: Build GraphQL APIs in VTEX IO apps with proper caching and authentication
  applies_to:
    - defining GraphQL schemas for VTEX IO apps
    - implementing resolver functions with ctx.clients
    - configuring @cacheControl and @auth directives
    - wiring resolvers into the Service entry point
  excludes:
    - backend service structure and middleware chains (see vtex-io-service-apps)
    - manifest and builder configuration (see vtex-io-app-structure)
  decision_scope:
    - "@cacheControl scope and maxAge selection"
    - "when to use @auth vs public access"
    - resolver-to-schema field mapping
