...
metadata:
  ...
  version: "1.0"
  purpose: Decide when to use section/component overrides and implement them safely via getOverriddenSection
  applies_to:
    - replacing native FastStore components
    - customizing component props
    - creating overridden sections
  excludes:
    - visual-only changes achievable with design tokens (use faststore-theming)
    - custom state management (use faststore-state-management)
    - data fetching and API extensions (use faststore-data-fetching)
  decision_scope:
    - override-vs-theming
    - component-replacement-vs-props-override
    - override-vs-custom-section
