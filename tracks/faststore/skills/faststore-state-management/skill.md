...
metadata:
  ...
  tags:
    ...
    - hooks
  globs:
    - "**/use{Cart,Session,Search}*.{ts,tsx}"
    - "**/*sdk*.{ts,tsx}"
  version: "1.0"
  purpose: Decide when to use SDK hooks and implement cart, session, and search state safely
  applies_to:
    - managing shopping cart state
    - reading and updating session data (locale, currency, channel)
    - building faceted search interactions
    - tracking analytics events
  excludes:
    - visual theming (use faststore-theming)
    - component overrides (use faststore-overrides)
    - GraphQL data fetching and API extensions (use faststore-data-fetching)
  decision_scope:
    - sdk-hooks-vs-custom-state
    - cart-validation-handling
