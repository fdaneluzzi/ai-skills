...
metadata:
  ...
  globs:
    ...
    - "**/*.tokens.scss"
  version: "1.0"
  purpose: Decide when to use design tokens vs overrides and implement visual customizations safely
  applies_to:
    - changing store colors, typography, spacing, borders
    - customizing component visual appearance via tokens
    - creating custom themes in src/themes/
  excludes:
    - behavioral changes to components (use faststore-overrides)
    - state management (use faststore-state-management)
    - data fetching (use faststore-data-fetching)
  decision_scope:
    - theming-vs-overrides
    - global-tokens-vs-local-tokens
    - data-attribute-selectors-vs-class-selectors
